 ...
 * Generates 50 fetal echo flashcards (varied difficulty) via AI gateway
...
if (!AI_GATEWAY_URL || !AI_GATEWAY_API_KEY) { console.error("AI gateway credentials not set"); process.exit(1); }
...
async function callAiGateway(topic, difficulty, count) {
...
Format: {"questions":[{"question":"...","reviewAnswer":"...","tags":["tag1","tag2"]}]}`;
  ...
  const resp = await fetch(`${AI_GATEWAY_URL}/v1/chat/completions`, {
    method: "POST",
    headers: {
  ...
  if (!resp.ok) {
    ...
    throw new Error(`AI gateway ${resp.status}: ${err.substring(0, 300)}`);
  }
...
async function main() {
  ...
  for (const batch of BATCHES) {
    ...
    try {
      const cards = await callAiGateway(batch.topic, batch.difficulty, batch.count);
