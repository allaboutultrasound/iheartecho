 ...
 * mediaUpload.test.ts
 *
 * Tests for the direct-to-R2 upload flow.
...
describe("R2 key helpers", () => {
  it("uses normalized object keys for uploads", () => {
    const fileKey = "media/test.zip";
    expect(fileKey.replace(/^\/+/, "")).toBe("media/test.zip");
  });
  ...
  it("handles nested folder paths", () => {
    ...
    expect(fileKey.replace(/^\/+/, "")).toBe("media/courses/acs/quiz.zip");
  });
});
...
describe("Direct upload flow", () => {
  it("prepare endpoint returns required fields", () => {
    ...
    const prepareResponse = {
      uploadUrl: "https://r2.example.test/presigned-upload",
      fileKey: "media/test-abc123.zip",
      publicUrl: "https://cdn.example.com/media/test-abc123.zip",
      method: "PUT",
      headers: { "Content-Type": "application/zip" },
    };
    expect(prepareResponse).toHaveProperty("uploadUrl");
    expect(prepareResponse).toHaveProperty("fileKey");
    expect(prepareResponse).toHaveProperty("publicUrl");
    expect(prepareResponse.method).toBe("PUT");
  });
  ...
  it("file only travels once in direct upload flow", () => {
    // In the old chunked flow: browser → server → storage (2 transfers)
    // In the new direct flow: browser → R2 (1 transfer)
...
describe("CORS configuration", () => {
  it("R2 bucket CORS supports browser PUT uploads", () => {
    const corsHeaders = {
      "access-control-allow-origin": "https://app.iheartecho.com",
      "access-control-allow-methods": "PUT",
      "access-control-allow-headers": "content-type",
    };
    expect(corsHeaders["access-control-allow-origin"]).toBe("https://app.iheartecho.com");
